...
**自动优化配置**（同构图检测后自动应用）:
- tau: 0.4 → 0.25 (增强对比学习区分度)
- num_cand_per_node: 5 → 15 (增加候选边)
- cand_sources: embed → embed,twohop,common (多种来源)
- lambda_cand_bce: 0.0 → 0.1 (增加重构损失)
- lambda_rec: 0.1 → 0.05
- adv_lambda: 1.0 → 1.2
- adv_temp: 1.0 → 0.8
- bias: 0.0001 → 0.001
...
**结果**:
- 节点分类: micro_f1=**21.61±0.71**, macro_f1=12.39±0.51, acc=21.61±0.71
- 有效节点: 10,703 / 13,178 (81.2%)
- CS-greedy 最优: **w=0.3, P=74.27, R=94.42, F1=81.22, Jaccard=72.09**, size=13.2
- 训练时间: 398.39s (训练) + 570.40s (测试) = 970.90s (总)

**性能提升**:
- 节点分类 Micro-F1: 2.55% → **21.61%** (+19.06 个百分点)
- 社区搜索 F1: 44.85% → **81.22%** (+36.37 个百分点) ⬆️⬆️⬆️
- 与 SLRL (87.10%) 的差距: 42.25% → **5.88%**

**备注**: 
- ✅ 同构图自动优化效果显著！
- ✅ 社区搜索 F1 达到 81.22%，非常接近 SLRL 的 87.10%
- ✅ Recall 高达 94.42%，说明模型能找到几乎所有社区成员
- ✅ Precision 74.27%，准确度也很高
- 节点分类仍然较低（21.61%），但社区搜索效果很好，说明嵌入质量足够好
...
**结果**:
- 节点分类: micro_f1=2.55±0.43, macro_f1=0.97±0.27, acc=2.55±0.43
- 有效节点: 10,703 / 13,178 (81.2%)
- CS-greedy 最优: w=0.0, P=59.04, R=44.67, F1=44.85, Jaccard=34.04, size=4.4
- 训练时间: 284.70s (训练) + 554.39s (测试) = 840.15s (总)

**备注**: 
- 节点分类效果极差（F1 只有 2.55%），可能是因为：
  1. 社区标签质量不高（SNAP 数据集的 ground truth 社区可能不准确）
  2. 图结构特征（度、对数度）不足以区分不同社区
  3. 社区数量太多（4,517 个社区），分类任务困难
- 社区搜索结果相对较好（F1=44.85），说明嵌入质量还可以
