...
- `encoder=hii`
- `num_epochs=200`
- **注意**: 必须使用 v1.0.27 或更早版本的 train_ig.py，v1.1.0 的"同构图自动优化"会降低性能
...
### 社区搜索结果 (Greedy)
```
[CS-greedy] w=0.3  P=74.27 R=94.42 F1=81.22 Jaccard=72.09 size=13.2
```
...
### 运行时间
```
训练时间=396.88s, 测试时间=575.19s, 总运行时间=974.22s
每轮均值=1.984s
```

### 重要说明
- ✅ v1.0.27 版本配置对 com-amazon 数据集是最优的
- 🔴 v1.1.0 添加的"同构图自动优化"会修改关键参数（tau, num_hidden, cand_sources 等），导致性能下降
- 如需复现此结果，使用 `git checkout b2cc5e0 -- train_ig.py` 恢复到 v1.0.27 版本
