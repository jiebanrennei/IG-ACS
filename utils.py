...
def _normalize_cs_features(x, mode):
    if mode == 'row_sum':
        ...
        raise ValueError(f"Unknown community-search feature normalization: {mode}")
    return x / denom
...
def _binarize_symmetric(adj_sp):
    """二值化 + 对称化 + 去自环, 返回 uint8 scipy 稀疏矩阵。"""
...
def _validate_sparsify_topk(k):
    if isinstance(k, bool) or not isinstance(k, (int, np.integer)) or k <= 0:
        raise ValueError(f"sparsify_topk must be a positive integer, got {k!r}")
    return int(k)


def _strip_npz_suffix(value):
    value = str(value).strip()
    return value[:-4] if value.endswith('.npz') else value


def _sparsify_topk(adj_sp, k):
    """每行先保留 top-k 邻居, 再取无向并集；最终度数可能超过 k。"""
    k = _validate_sparsify_topk(k)
    csr = adj_sp.tocsr()
    n = csr.shape[0]
    if n == 0:
    ...
    for i in range(n):
        ...
        if end - start <= k:
            selected = csr.indices[start:end]
        else:
            vals = csr.data[start:end]
            topk_idx = np.argpartition(vals, -k)[-k:]
            selected = csr.indices[start:end][topk_idx]
        cols.append(selected)
        rows.append(np.full(len(selected), i, dtype=np.int64))
    ...
    return _binarize_symmetric(sparse)
...
def get_cs_dataset(root, name, meta_path=None, multi_relation=False,
    ...
    Args:
        ...
        cs_relations: 多关系模式下可选的 meta-path 名列表 (如 ['pap']);
                      None 表示用全部 cfg['meta_paths']。
        sparsify_topk: int or None. 非 None 时, 对平均度数超过此值的稠密 meta-path
                       自动做 top-k 稀疏化。每行先选 top-k 后取无向并集。
        cs_full_graph: True 时额外构建 data.cs_edge_index —— 全量合并所有
                       cfg['meta_paths'] (不稀疏化) 的无向图, 专供社区搜索的贪婪/
                       AC 扩展使用。
        data_dir: 精确数据目录覆盖；用于加载 raw-format 扰动目录。
    Returns:
        list containing one PyG Data object. 多关系模式下 data 额外带有
        edge_index_list / num_relations / relation_names 属性。
    """
    if name not in CS_DATASETS:
        available = ', '.join(sorted(CS_DATASETS))
        raise ValueError(f"Unknown community-search dataset {name!r}; available: {available}")

    cfg = CS_DATASETS[name]
    base = osp.abspath(osp.expanduser(data_dir)) if data_dir else osp.join(root, cfg['dir'])

    if sparsify_topk is not None:
        sparsify_topk = _validate_sparsify_topk(sparsify_topk)
    ...
    for mp in paths_to_merge:
        m = (sp.load_npz(osp.join(base, mp)) > 0).astype(np.uint8)
    ...
    if cs_full_graph:
        ...
        for mp in cfg['meta_paths']:
            m = (sp.load_npz(osp.join(base, mp)) > 0).astype(np.uint8)
            cs_adj = m if cs_adj is None else cs_adj.maximum(m)
    ...
    if multi_relation:
        ...
        for mp in rel_paths:
            ...
            relation_names.append(_strip_npz_suffix(mp))
