...
if __name__ == '__main__':
    ...
    if args.perturbed_data is None:
    ...
    if args.perturbed_data is not None:
        ...
        # 检查是目录还是文件
        if os.path.isdir(args.perturbed_data):
            info_file = os.path.join(args.perturbed_data, 'pert_info.json')
            data_file = os.path.join(args.perturbed_data, 'data.pt')

            # 加载扰动信息
            if os.path.exists(info_file):
                with open(info_file, 'r') as f:
                    pert_info = json.load(f)
                print(f"Perturbation info:")
                print(f"  Type: {pert_info.get('type', 'unknown')}")
                print(f"  Edge injection ratio: {pert_info.get('edge_noise_ratio', 0):.4f}")
                print(f"  Edge dropout ratio: {pert_info.get('edge_drop_ratio', 0):.4f}")
                print(f"  Feature noise std: {pert_info.get('feature_noise_std', 0):.4f}")
                print(f"  Seed: {pert_info.get('seed', 'N/A')}")
                perturbed_dataset_name = pert_info.get('dataset', args.dataset)
            else:
                perturbed_dataset_name = args.dataset

            cfg = CS_DATASETS[perturbed_dataset_name]
            if os.path.exists(data_file):
                # 旧格式：单个 data.pt 文件。若目录同时包含新格式文件，优先使用新格式。
                raw_format_files = [cfg_file for cfg_file in (
                    'pap.npz', 'psp.npz', 'mam.npz', 'mdm.npz'
                ) if os.path.exists(os.path.join(args.perturbed_data, cfg_file))]
                has_raw_format = (
                    os.path.exists(os.path.join(args.perturbed_data, cfg['feat']))
                    and len(raw_format_files) >= len(cfg['meta_paths'])
                    and os.path.exists(os.path.join(args.perturbed_data, 'labels.npy'))
                )
            ...
            if data is None:
                # 新格式：目录包含与原始 datasets/ 相同格式的文件
                # 直接用 get_cs_dataset 的逻辑加载
                import scipy.sparse as sp
                from utils import CS_DATASETS, _binarize_symmetric, _sparse_to_edge_index

                cfg = CS_DATASETS[perturbed_dataset_name]
                pert_dir = args.perturbed_data

                # 加载特征
                feat_sp = sp.load_npz(os.path.join(pert_dir, cfg['feat']))
                x = torch.from_numpy(feat_sp.toarray()).float()
                row_sum = x.sum(dim=1, keepdim=True).clamp(min=1.0)
                x = x / row_sum

                # 加载标签
                labels = np.load(os.path.join(pert_dir, 'labels.npy'))
                ...
                for mp in paths_to_merge:
                    m = sp.load_npz(os.path.join(pert_dir, mp))
                ...
                print(f"  [cs_full_graph] merged {cfg['meta_paths']} -> "
                      f"{data.cs_edge_index.size(1)} directed edges")
                ...
                print(f"\nLoaded from perturbed directory files")
    ...
    # 根据显存调整阈值: 全图 GCN 显存占用 = O(E × hidden_dim)
    # 即使 24GB, 1M+ 边的全图 GCN 也会 OOM
    if gpu_memory_gb >= 20:
        # 24GB GPU: 节点 >80K 或 边 >500K 就采样 (保守)
        node_threshold = 80000
        edge_threshold = 500000
        target_nodes = 60000
        print(f"[data] Using high-memory thresholds (80K nodes, 500K edges)")
    else:
        # 12GB GPU: 节点 >50K 或 边 >300K 采样
        node_threshold = 50000
        edge_threshold = 300000
        target_nodes = 40000
        print(f"[data] Using standard-memory thresholds (50K nodes, 300K edges)")

    # 大图自动子图采样
    should_sample = (num_nodes > node_threshold) or (num_edges > edge_threshold)
    if should_sample:
        target_nodes = min(target_nodes, num_nodes // 2)
